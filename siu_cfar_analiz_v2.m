...
%  ═══════════════════════════════════════════════════════════════
%
%  [D4] Bu analiz slow-time güç otokorelasyonunu ölçer.
%  CFAR range boyunca uygulandığı için buradaki korelasyon doğrudan
%  range-CFAR i.i.d. ihlali anlamına gelmez. Ancak slow-time
%  korelasyon, darbe entegrasyonu sonrası hücre başına güç
%  istatistiğinin efektif serbestlik derecesini değiştirir; bu da
%  CFAR eşik davranışını dolaylı olarak etkiler.
%  ═══════════════════════════════════════════════════════════════
...
xlabel('Gecikme (lag)'); ylabel('\rho(lag)');
title('(a) MTI Öncesi', 'FontWeight', 'bold');
...
xlabel('Gecikme (lag)'); ylabel('\rho(lag)');
title('(b) MTI Sonrası', 'FontWeight', 'bold');
legend('Location', 'northeast', 'FontSize', 12);
ylim([-0.3 1.0]); grid on; box on;
...
N_kal = 1500;
T_UST_SINIR = 50.0;  % [D14] Sınır kontrolü için sabit
...
% [D16] Kalibrasyon doğrulama — T_faktor ile eşleşen gerçek Pfa
...
fprintf('├───────────┼──────────────┼──────────────┼──────────────┼──────────────┤\n');
% Pfa üst sınırı (0 gözlendiğinde)
...
set(gca, 'XTick', 1:n_gurultu, 'XTickLabel', gurultu_adi);
set(gca, 'YTick', 1:n_cfar, 'YTickLabel', cfar_adi);
xlabel('Gürültü Tipi', 'FontWeight', 'bold');
ylabel('CFAR Türü', 'FontWeight', 'bold');

for ci = 1:n_cfar
    for ni = 1:n_gurultu
        oran = pfa_oran(ci, ni);
        if Pfa_olculen(ci,ni) < pfa_ust_sinir
            % Ölçüm çözünürlüğünün altında
            renk = 'w';  % Siyah arka plan → beyaz metin
...
%  ═══════════════════════════════════════════════════════════════
%
%  [D1]  JSR = P_jam/P_sig → sabit. Jammer gücü = JSR × SNR × P_n
%  [D11] Her gürültü tipi için hedefsiz koşum → gerçekleşen Pfa ölçülür.
%        Bu, Pd farkının jammer etkisinden mi yoksa eşik
%        uyumsuzluğundan mı geldiğini ayırt etmeye yarar.
%
%  Sabit-JSR senaryosu: Hedef güçlenince jammer de aynı oranda
%  güçlenir. Bu, EW ortamında jammerın hedef mesafesini bildiği
%  ve gücünü buna göre ayarladığı senaryoyu temsil eder.
%  ═══════════════════════════════════════════════════════════════
...
% [D11] Gerçekleşen Pfa: aynı jammer senaryosunda hedef olmadan
Pfa_gercek = zeros(n_gurultu, n_snr);
N_mc_pfa_kontrol = 400;  % Pfa için ayrı MC koşumu
% 400 × 42 hücre = 16800 test/nokta → beklenen FA ≈ 16.8 (Pfa=1e-3)
...
for ni = 1:n_gurultu
    ...
    for si = 1:n_snr
        ...
        jam_guc = JSR_sabit_lin * snr_lin;  % [D1]

        % --- Pd ölçümü (hedefli) ---
        tespit = 0;
        for mc = 1:N_mc_pd
            veri = complex(zeros(N_menzil, N_darbe));  % [D13]
            for r = 1:N_menzil
                tmp = generate_colored_noise(N_darbe, alpha);
        ...
        % --- [D11] Gerçekleşen Pfa (hedefsiz, aynı jammer) ---
        fa_sayac = 0;
        top_test = 0;
...
% [D11] Pfa kontrol tablosu (ortalama, min, maks)
...
fprintf('  Not: Sabit-JSR senaryosunda jammer gücü SNR ile değişir,\n');
fprintf('       bu yüzden Pfa SNR noktalarına göre farklılık gösterebilir.\n')

%% Ortak stil ayarları
lw_main   = 2.2;                 % ana eğriler
lw_ref    = 1.4;                 % referans çizgileri
ms_main   = 5;                   % marker boyutu
fs_axis   = 24;
fs_title  = 14;
fs_legend = 24;
...
xlabel(ax1, 'SNR (dB)','FontSize', 18);
ylabel(ax1, 'Tespit Olasılığı, P_d','FontSize', 18);
...
% Ölçüm çözünürlüğü: 0 gözlendiğinde gösterilecek taban
pfa_olcum_tabani = 1 / (N_mc_pfa_kontrol * test_hucre_sayisi);

for ni = 1:n_gurultu
    % Sıfır gözlenen noktaları ölçüm tabanında göster
...
xlabel(ax2, 'SNR (dB)','FontSize', 18);
ylabel(ax2, 'Yanlış Alarm Oranı, P_{fa}','FontSize', 18);
...
for ni = 1:n_gurultu
    ...
    for ji = 1:n_jsr
        ...
        jam_guc = jsr_lin * SNR_sabit_lin;  % [D1]
        tespit = 0;
...
xlabel(ax4, 'JSR (dB)', 'FontSize', 18);
ylabel(ax4, 'Tespit Olasılığı, P_d','FontSize', 18);
...
% MTI notch bölgesi — filtre cevabından türetilmiş
% |H(f)|² < 1 (0 dB) olan bölge → bastırma bölgesi
% v = f_d * lam / 2, f_d_norm = f_d / PRF
w_notch = linspace(0, pi, 1000);
H_notch = freqz(mti_koef, 1, w_notch);
H_pow_notch = abs(H_notch).^2;
% Eşik: |H|² = 1 → 0 dB (birim kazanç altı = bastırma)
f_norm_notch = w_notch / (2*pi);  % 0..0.5
v_notch_all = f_norm_notch * PRF * lam / 2;  % m/s
idx_notch = find(H_pow_notch >= 1.0, 1, 'first');
if ~isempty(idx_notch)
    v_notch_sinir = v_notch_all(idx_notch);
else
    v_notch_sinir = 3;  % varsayılan
end
...
xlabel('Hedef Radyal Hızı, v_r (m/s)', 'FontWeight', 'bold');
ylabel('Dedeksiyon Olasılığı, P_d', 'FontWeight', 'bold');
title(sprintf('Şekil 5. P_d - Hedef Hızı (SNR = %d dB, JSR = %d dB, CA-CFAR)', ...
    SNR_hiz, JSR_hiz), 'FontWeight', 'bold');
...
%% Ortak stil ayarları
lw_main   = 2.6;                 % ana eğriler
lw_ref    = 1.4;                 % referans çizgileri
ms_main   = 5.5;                 % marker boyutu
fs_axis   = 24;                  % eksen başlıkları
fs_tick   = 24;                  % eksen sayıları
fs_title  = 14;
fs_legend = 24;
ref_col   = [0.55 0.55 0.55];
grid_col  = [0.82 0.82 0.82];
...
xlabel(ax6, 'JSR (dB)', 'FontSize', 24);
ylabel(ax6, 'Tespit Olasılığı, P_d', 'FontSize', 24);
...
%% ═══════════════════════════════════════════════════════════════
%  ÖZET — [D3] Tamamı veriden üretilmiş
%  [D20] Beraberlik, inf, üst sınır durumları düzeltildi
%  ═══════════════════════════════════════════════════════════════
...
% Kahverengi ve Mavi için en düşük Pfa sapması gösteren CFAR
...
for ni_t = [3, 4]
    ...
    % [D1-fix] Tüm sütunu kontrol et, sadece min'i değil
    if all(sutun_oranlari < pfa_ust_oran)
        % Hepsi ölçüm tabanının altında → ayırt edilemez
...
fprintf('   → Pfa sapması düşükse Pd farkı ağırlıklı olarak jammer\n');
fprintf('     etkisinden kaynaklanır; yüksekse eşik uyumsuzluğu da katkıda\n');
fprintf('     bulunur.\n');
fprintf('   Not: Bölüm 2 (sadece jammer) ile Bölüm 3 (jammer+thermal) Pfa\n');
fprintf('   değerleri arasındaki fark normaldir. Düşük SNR''de jammer gücü\n');
fprintf('   küçüktür (P_j = JSR × SNR × P_n), termal gürültü baskındır ve\n');
fprintf('   beyaz gürültü benzeri davranış görülür.\n');
...
% inf olanları "Pd'ye ulaşamadı" olarak raporla
...
% [D4-fix] -inf olanları ayır, sadece sonlu değerler arasında kıyasla
...
% MTI_FILTRE_MATRIS  Her menzil hücresinde yavaş-zamanda MTI filtresi.
%   veri_2d : [N_menzil × N_darbe]
%   koef    : FIR filtre katsayıları
%   cikis   : [N_menzil × (N_darbe - length(koef) + 1)]
...
% CFAR_REF_HESAPLA  CFAR referans seviyesi.
%   cfar_tipi: 1=CA, 2=GO, 3=SO, 4=OS (%75 persentil)

    guc_profil = guc_profil(:);  % [D7]
    ...
    switch cfar_tipi
        ...
        case 4  % OS-CFAR (%75 persentil — bildiri'de gerekçelendirilecek)
